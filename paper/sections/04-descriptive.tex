...
\emph{Almost nothing places aggregates by bearing.} Labelling techniques tie
individual labels to bearing and resolve conflicts by dropping them; rings that
aggregate order their marks by category or time. The cells in which binning is
by bearing \emph{and} placement honours that bearing \emph{and} marks are
aggregates are, among the techniques coded, occupied only by this work and, for
whole-map necklaces, by necklace maps themselves.
