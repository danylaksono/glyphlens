...
Two parts of the solver are approximate by construction. When the marks nearly
fill the cycle, the wrap-around constraint caps the span of $y$. The
implementation enforces it by alternating projection between monotone sequences
and sequences of bounded span, which converges to a feasible point but is not
guaranteed to reach the constrained optimum. Feasible intervals are enforced by
clamping and re-solving for a fixed number of passes. For the first, the exact
solution is cheap. The KKT conditions of the span-capped problem are those of
plain isotonic regression on targets whose two end points are pulled inward by
$\mu/w$, so a bisection on the multiplier $\mu$ solves it. Against that exact
solver, the implementation matched the order-preserving optimum (relative
tolerance $10^{-6}$) in all 1,800 closed-ring instances of the check,
including the 40\% in which the cap binds, and no output overlapped. That is a
test, not a proof. Interval handling was not tested.
\todo{Replace alternating projection with the exact multiplier bisection (a
dozen lines), and give intervals an exact method or a benchmarked gap.}
