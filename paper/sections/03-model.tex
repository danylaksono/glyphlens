...
\doubt{The implementation assumes that ordering marks by preferred position is
optimal (``any crossing solution can be uncrossed without increasing cost''). That
is standard for equal widths but unproven here for unequal $h_k$ and $w_k$. For
the related max-scale necklace problem with centroid intervals, Speckmann and
Verbeek show that the order by region bearing is \emph{not} generally optimal
(it is a tight $\frac12$-approximation)~\cite{speckmann2015algorithms}; their
objective and constraints differ from \cref{eq:placement}, so this is a warning,
not a counterexample. A proof or a counterexample search is needed.}
