...
(another lens, time or region). The baseline is itself a choice---the
...
\begin{equation}
  ...
  \quad\text{s.t.}\quad
  \text{the arcs } [x_k-h_k,\,x_k+h_k] \text{ are disjoint},\;
  x_k\in I_k ,
  \label{eq:placement}
\end{equation}
the necklace-map placement problem of Speckmann and Verbeek
\cite{speckmann2010necklace,speckmann2015algorithms} with a squared-displacement
objective. For a fixed cyclic order and no intervals, substituting
$y_k=x_k-c_k$, where $c_k$ accumulates the minimum spacings $h_{k-1}+h_k$, turns
non-overlap into monotonicity of $y$, so the optimum is a weighted isotonic
regression, solved exactly in $O(K)$ by pool-adjacent-violators. The implementation
orders marks by preferred position, tries each of the $K$ places to cut the
cycle, and keeps the cheapest, for $O(K^2)$ overall.
\doubt{The claim that the order by preferred position is optimal (``any
crossing solution can be uncrossed without increasing cost'') is asserted in
the code but not proven here; it is standard for equal widths, and needs either
a proof for unequal $h_k$ and $w_k$ or a counterexample search. Speckmann and
Verbeek report that the \emph{any-order} problem is NP-hard (with intervals), so
the argument must lean on the absence of intervals.}

Two parts of the solver are approximate, and we say so rather than hide it.
When the marks nearly fill the cycle, the wrap-around constraint is enforced by
alternating projection between monotone sequences and sequences of bounded
span, which converges to a feasible point but not, in general, to the
projection itself (Dykstra's algorithm would); and feasible intervals are
enforced by clamping and re-solving for a fixed number of passes. Both are exact
when inactive, which is the common case for a lens with a few dozen bins.
\todo{Either implement Dykstra's algorithm for the bounded-span case and an
exact interval method, or keep the approximation and report its gap on a
benchmark.}
...
begun to encode this: Honeycomb Plots fit a density gradient inside each tile
and draw it as a ``diamond cut''~\cite{trautner2022honeycomb}, and HexTiles
encode within-tile variability as a per-tile confidence, argued explicitly as
mitigation of the modifiable areal unit problem~\cite{kawakami2024hextiles}.
\doubt{Both characterisations to be checked against the full texts; see the
citation audit.}
...
We do not claim the statistic. Counting within growing discs around a point is
the idea behind point-centred second-order analysis~\cite{getis1987second}
and the $K$ function it localises~\cite{ripley1977modelling}; under complete
spatial randomness $N\propto r^2$ and $E=2$, which is the local
$L$-function's $L(r)=r$ expressed as a log-log slope. The same slope around a
chosen centre is the mass--radius dimension used in fractal urban
analysis~\cite{yamu2016fractal}. Three things distinguish the lens's use of it:
...
\todo{Report $E$ relative to the selection's own area elasticity,
$E/(d\ln A/d\ln r)=\lambda_{\mathrm{edge}}/\bar\lambda$, which is 1 under uniform
density for every shape; and add a Monte-Carlo envelope under a null model
\cite{wiegand2004rings} instead of the bare reference value.}
\doubt{The equivalences in this paragraph are derived here; the specific
readings of Getis \& Franklin, Ripley and Yamu et al.\ are pending the citation
audit.}
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \node[font=\scriptsize, anchor=west, text=black!55] at (axis cs:40,7.3) {too few members};
      \node[font=\scriptsize, anchor=south west, text=black!60] at (axis cs:2100,1.95) {uniform: $2-b$};
    \end{axis}
  \end{tikzpicture}
  ...
  OpenStreetMap extract by the library and plotted from its output. The shaded
  span has fewer than 30 members and is not drawn. The early cliff ($\hat E\approx7.3$
  at 475--500\,m) says a band of places at that distance is far denser than
  anything inside it, so a radius chosen there is fragile; a second, weaker
  rise follows at 925--1000\,m; and beyond about 1.4\,km the curve stays below
  the uniform reference, meaning that widening the lens adds ground faster
  than it adds places.}
