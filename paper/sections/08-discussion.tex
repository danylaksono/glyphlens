...
without careful design~\cite{borgo2013glyph,fuchs2017glyph}. The title and
abstract are therefore narrowed to what every demonstration shows: one
categorical variable (four classes of place) summarised by count, from points
or from district tables. A genuinely multivariate case, such as several census
deprivation domains, is future work, and the claim should widen only with
it.
...
We turned the equivalence into a check. The selection takes a distance-decay
kernel (box-car, bi-square, or the Gaussian that Dykes and Brunsdon
use~\cite{dykes2007gwvis}), and it multiplies each member's weight by $K(d/h)$.
With categorical binning and share normalisation, a field then computes GW
proportions. On the bundled extract, at the 241 centres of a hexagonal lattice
with $h = 600$\,m, it agreed with the direct formula over all 1{,}449 places
exactly with the box-car kernel and to $5\times10^{-16}$ with the bi-square
(\texttt{paper/scripts/gw-check.mjs}). A lens has a radius and the Gaussian
does not have compact support, so the field truncates it: the largest error in a
proportion was 0.09 at a radius of $2h$, 0.01 at $3h$ and $5\times10^{-4}$ at
$4h$. The equivalence is a validity check on the implementation, not a
contribution. It also makes the field a GW visualization with the lens's
bearing decomposition added.
...
control shows how fragile the reading is. What we have shown is its use on
categorical counts; whether it helps anyone is the question the planned study
exists to answer.
